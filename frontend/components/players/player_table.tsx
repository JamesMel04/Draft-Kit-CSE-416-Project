...
function PitchersTable({ players } : {players : PitcherPlayer[]}) {
  ...
  const visiblePlayers = useMemo(() => {
    const sorted = [...players].sort(sortPitchers);
    if (!mlbOnly) return sorted;
    return sorted.filter((p) => p.fantasyPositions?.includes("P"));
  }, [players, sort, mlbOnly]);
...
function HittersTable({ players }: { players: HitterPlayer[] }) {
  ...
  const visiblePlayers = useMemo(() => {
    const sorted = [...players].sort(sortHitters);
    if (!selectedPositions.length) return sorted;
    return sorted.filter((p) =>
      p.fantasyPositions?.some((pos) => selectedPositions.includes(pos))
    );
  }, [players, sort, selectedPositions]);
